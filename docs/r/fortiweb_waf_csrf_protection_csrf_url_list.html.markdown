...
layout: "fortiadc"
page_title: "FortiWEB: fortiweb_waf_csrf_protection_csrf_url_list"
description: |-
  Configure FortiWEB waf csrf protection csrf url list configuration.
---
...
## Example Usage
```hcl
resource "fortiweb_waf_csrf_protection_csrf_url_list" "test" {
	sub_mkey = "1"
	mkey = "test"
	host_status = "enable"
	host = "test"
...
* `sub_mkey` - The id of the waf csrf protection csrf url list entry.
...
Waf Csrf Protection Csrf Url List can be imported using any of these accepted formats:
```
$ terraform import fortiweb_waf_csrf_protection_csrf_url_list.labelname {mkey}
```
